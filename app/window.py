...
from .pdf_highlights import PdfHighlight
...
from .tag_flow import merged_tag_rows as merged_tag_rows
...
"""
    ...
    def _translate_selection(self, text: str):
        return translation_flow.translate_selection(self, text)

    def _ensure_translate_dialog(self) -> TranslationDialog:
        return translation_flow.ensure_translate_dialog(self)

    def _run_translation(self, selection: str, provider: str, target: str, force: bool=False):
        return translation_flow.run_translation(self, selection, provider, target, force)

    def _on_retranslate_requested(self, provider: str, target: str, force: bool):
        return translation_flow.on_retranslate_requested(self, provider, target, force)

    def _on_translation_done(self, request_id: int, result: str, provider: str, target: str):
        return translation_flow.on_translation_done(self, request_id, result, provider, target)

    def _on_translation_failed(self, request_id: int, message: str):
        return translation_flow.on_translation_failed(self, request_id, message)
...
"""
    ...
    def _open_pdf(self, path: Path):
        return pdf_flow.open_pdf(self, path)

    # --- PDF page notes ---
    def _refresh_pdf_notes_panel(self):
        return pdf_flow.refresh_pdf_notes_panel(self)

    def _save_pdf_notes(self):
        return pdf_flow.save_pdf_notes(self)

    def _pdf_add_note(self):
        return pdf_flow.pdf_add_note(self)

    def _pdf_add_note_at_page(self, page: int):
        return pdf_flow.pdf_add_note_at_page(self, page)

    def _find_pdf_note(self, note_id):
        return pdf_flow.find_pdf_note(self, note_id)

    def _pdf_note_activated(self, note_id):
        return pdf_flow.pdf_note_activated(self, note_id)

    def _pdf_edit_note(self, note_id):
        return pdf_flow.pdf_edit_note(self, note_id)

    def _pdf_delete_note(self, note_id):
        return pdf_flow.pdf_delete_note(self, note_id)

    # --- PDF text highlights ---
    def _pdf_find(self):
        return pdf_flow.pdf_find(self)

    def _on_pdf_pen_mode_changed(self, on: bool):
        return pdf_flow.on_pdf_pen_mode_changed(self, on)

    def _toggle_pen_mode(self):
        return pdf_flow.toggle_pen_mode(self)

    def _refresh_pdf_highlights_panel(self):
        return pdf_flow.refresh_pdf_highlights_panel(self)

    def _save_pdf_highlights(self):
        return pdf_flow.save_pdf_highlights(self)

    def _on_pdf_highlight_requested(self, payload):
        return pdf_flow.on_pdf_highlight_requested(self, payload)

    def _find_pdf_highlight(self, hid):
        return pdf_flow.find_pdf_highlight(self, hid)

    def _pdf_highlight_activated(self, hid):
        return pdf_flow.pdf_highlight_activated(self, hid)

    def _pdf_highlight_recolor(self, hid, color):
        return pdf_flow.pdf_highlight_recolor(self, hid, color)

    def _pdf_highlight_edit_note(self, hid):
        return pdf_flow.pdf_highlight_edit_note(self, hid)

    def _pdf_highlight_edit_text(self, hid, text):
        return pdf_flow.pdf_highlight_edit_text(self, hid, text)

    def _pdf_highlight_delete(self, hid):
        return pdf_flow.pdf_highlight_delete(self, hid)

    # --- embedded (Adobe-authored) PDF annotations: read-only ---
    def _refresh_pdf_embedded_annotations_panel(self):
        return pdf_flow.refresh_pdf_embedded_annotations_panel(self)

    def _pdf_embedded_annotation_activated(self, entry):
        return pdf_flow.pdf_embedded_annotation_activated(self, entry)

    # --- writing replies/edits back into the PDF (incremental saves only) ---
    def _pdf_annotation_author(self) -> str:
        return pdf_flow.pdf_annotation_author(self)

    def _edit_pdf_annotation_author(self):
        return pdf_flow.edit_pdf_annotation_author(self)

    def _refresh_pdf_annotation_write_state(self):
        return pdf_flow.refresh_pdf_annotation_write_state(self)

    def _perform_pdf_annotation_write(self, action, focus_xref) -> bool:
        return pdf_flow.perform_pdf_annotation_write(self, action, focus_xref)

    def _pdf_annotation_reply(self, entry, text: str) -> bool:
        return pdf_flow.pdf_annotation_reply(self, entry, text)

    def _pdf_annotation_edit(self, entry, text: str) -> bool:
        return pdf_flow.pdf_annotation_edit(self, entry, text)

    def _annotation_replies_to(self, entry) -> list:
        return pdf_flow.annotation_replies_to(self, entry)

    def _confirm_annotation_delete(self, entry, replies) -> bool:
        return pdf_flow.confirm_annotation_delete(self, entry, replies)

    def _pdf_annotation_delete(self, entry) -> bool:
        return pdf_flow.pdf_annotation_delete(self, entry)

    def _on_pdf_embedded_annotation_clicked(self, entry):
        return pdf_flow.on_pdf_embedded_annotation_clicked(self, entry)
    ...
    def _index_doc_tags(self, path):
        return tag_flow.index_doc_tags(self, path)
    ...
    def _update_front_tags(self):
        return tag_flow.update_front_tags(self)

    def _refresh_tags_panel(self):
        return tag_flow.refresh_tags_panel(self)

    def _set_pdf_panel_document(self, path):
        return pdf_flow.set_pdf_panel_document(self, path)

    # --- document-level tag management (MD + PDF) ---
    def _open_manage_tags(self, paths):
        return tag_flow.open_manage_tags(self, paths)

    def _on_doc_tags_changed(self, paths):
        return tag_flow.on_doc_tags_changed(self, paths)

    def _delete_tag(self, tag: str) -> None:
        return tag_flow.delete_tag(self, tag)

    def _rename_tag(self, old: str, new: str | None=None) -> None:
        return tag_flow.rename_tag(self, old, new)

    def _add_tag_to_paths(self, paths):
        return tag_flow.add_tag_to_paths(self, paths)

    def _assign_tag_to_paths(self, tag: str, paths):
        return tag_flow.assign_tag_to_paths(self, tag, paths)

    def _on_tag_selected(self, tag: str):
        return tag_flow.on_tag_selected(self, tag)
