...
分支 `feature/large-markdown-render`，工作樹 `.claude/worktrees/batch3-large-markdown`（自 `main` = 26d17e0，後續 merge main = 520fd01）。
本批未改首頁 HTML／`app/window.py`，以免與另一批未合併分支衝突（首頁新增與 PDF 註解批次已由 merge 併入）。
...
- 啟動方式：原始碼 `py -3 -X utf8 -m app.render_worker <port>`；
  封裝 `MarkdownViewer.exe --render-worker <port>`（`main.py` 於 `__main__` 最前面攔截，早於任何 Qt 工作）。
  **token 不走 argv**（同機任何程序都能讀 CommandLine）：父程序把 32 bytes token 寫進子程序 stdin 後關閉，
  子程序從 stdin 讀（frozen windowed 下 `sys.stdin` 可能是 None，退回 fd 0）；仍是「先比對 token 再 `pickle.loads`」，
  不符就 kill 並且完全不讀 frame。
...
| `app/render_worker.py`（新增） | 子程序服務迴圈；stdin token；原始碼模式只 import markdown-it／Pygments（**封裝模式下子程序就是本體 exe，`main.py` 的模組層 import 仍會載入 Qt，子程序較重**） |
...
| `app/renderer.py` | 部分載入搜尋落空時的說明與自動重搜（`_notify_partial_search_miss()`、`partial_search_missed` 訊號）、`_RenderSignals.partial_ready`、`_MarkdownRenderWorker._emit_fast_preview()`、`RendererView._on_file_partial_ready()`／`_stale_render()`／`is_partial_preview()`、完整版接上時還原被部分頁吃掉的 pending find／scroll、部分預覽期間延後 PDF 匯出 |
| `main.py` | `--render-worker` argv 入口（封裝版路徑） |
| `markdown_viewer.spec` | `hiddenimports` 加 `app.render_service`／`app.render_worker` |
| `tools/benchmark_markdown_first_load.py` | 新增 `first_readable_once()`、`cancel_switch_once()`、`--first-readable-reps`／`--cancel-switch-reps`、JSON `first_readable`／`cancel_switch`；**並修正** `parser_lock_contention_once()`：`big_still_running_when_small_finished` 改在小檔請求返回當下取樣（原本在 `thread.join()` 之後讀，永遠是 False，害衍生比例恆為 1.0） |
...
| 5 MB 解析中切到 100 KB：1 秒內、不等舊任務 | **pass** | 100 KB 請求延遲 3215.2 → **36.3 ms**（10 MB：8783.4 → 36.1 ms），與單獨開啟 34.9 ms 幾乎相同。旗標修好後重量：`small_blocked_until_big_finished_fraction` **0.0**（p50 31.6 ms／10 MB 39.2 ms）；同一份程式碼設 `MDV_DISABLE_RENDER_SUBPROCESS=1` 的對照組（＝舊行為）為 **1.0**（p50 3161 ms），證據 `docs/benchmarks/large-markdown-contention-inprocess-control-2026-09-08.json` |
...
| 部分載入時搜尋不可靜默失敗 | **pass** | 前綴頁搜尋落空時在橫幅加一行「找不到「X」。目前只載入這份文件的前 N%…」，並自動把同一組搜尋排到完整預覽載入後重跑（`test_search_miss_on_a_partial_page_explains_the_loaded_range`） |
...
| 全套 `pytest tests` exit 0 | **pass** | 1592 passed／75 skipped（合併 main 520fd01 後；含本批新增 48 項，以及自基線批次帶入的量測工具自我測試 6 項） |
...
4b. **封裝模式的子程序會載入 Qt**：子程序入口在 `main.py`，其模組層 import 會拉進 PySide6，
   啟動比原始碼模式（實測 ~205 ms）慢，實際數字未量（未打包）。
...
7. 快速模式沒有做成使用者可切換的開關（門檻自動判斷）；規格允許的「明確兩選項」目前是
   「自動顯示部分內容 + 明講範圍 + 搜尋落空時明講並自動重搜 + 自動接上完整版」。
8. **前綴切割不處理縮排式（4 空格）程式區塊的內部空行**：`fast_preview_split()` 只追蹤
   fence／front matter／`$$`／`<details>`，縮排 code block 中的空行仍算邊界，
   切點後半段可能少一段縮排程式碼（前綴頁已標示為部分內容，完整版會補回，未見輸出錯誤）。
9. **IPC frame 上限 256 MB** 是防呆用的長度合理性檢查，不是記憶體預算；10 MB 文件的 body
   可達原文數倍，真正的保護是 `MAX_PREVIEW_BYTES`（10 MB）與子程序回收。
10. `partial_search_missed` 訊號目前沒有接到 `app/window.py`（避免與其他批次衝突）；
   提示以頁內橫幅呈現，狀態列整合留待之後。

## 5. 意外發現

1. **基線工具的 `small_blocked_until_big_finished_fraction` 旗標本來是壞的**：
   `parser_lock_contention_once()` 在 `thread.join()` **之後**才讀 `big_done.is_set()`，
   所以永遠是 `False`、比例永遠 1.0，before／after 都一樣。已於本批修正（改在小檔請求返回當下取樣）
   並重跑該欄位：新架構 0.0、`MDV_DISABLE_RENDER_SUBPROCESS=1` 對照組 1.0。
   `docs/benchmarks/large-markdown-after-2026-09-08.json` 只有這一節是重跑的（檔內 `limitations` 已註明），
   其餘數字仍是同日同機的第一次 after 量測。
...
```powershell
py -3 -X utf8 -m pytest tests -q -p no:cacheprovider --basetemp tmp/next-render-fix
# 1592 passed, 75 skipped, exit 0
...
py -3 -X utf8 -m pytest tests/test_renderer_async.py tests/test_renderer_source_navigation.py `
    ...
    --basetemp tmp/next-render-fix-we   # 10 passed（RUN_WEBENGINE_TESTS=1 的 renderer 兩檔）
