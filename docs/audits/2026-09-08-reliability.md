...
> 狀態回填（2026-09-21）：本文保留 2026-09-08 的重現紀錄，不能當作目前開放清單。R1／R2／R3 已於 **v1.31.0** 解決；R6 已於 **v1.32.0** 解決；R4 已在本輪 Unreleased 增加差異與保留雙方；R5／R7／R8 仍開放。版本依據：[CHANGELOG](../../CHANGELOG.md)。
...
**已於 v1.31.0 解決**：支援的 Markdown 相對資源會重算，無法可靠重算或跨磁碟情境保守拒絕；不代表所有語法都能搬移。
...
**已於 v1.31.0 解決**：主檔、sidecar 與備份納入同次預檢／回滾操作。
...
**已於 v1.31.0 解決**：啟動提供待復原草稿清單，含未在工作階段及原檔已不存在的項目。
...
**已於本輪 Unreleased 提供**唯讀差異與保留雙方；預覽行內編輯仍需先結束該次編輯才能使用完整文件比較。
...
**已於 v1.32.0 解決**：非模態下載、進度、取消及稍後安裝；以下為修正前紀錄。
